...
# One convenience function to get the resultant length.
getR <- function(th) sqrt(sum(cos(th)) ^ 2 + sum(sin(th)) ^ 2)
...
plotCircular <- function(th, alpha = 0.35, inputtype = "radians",
                         outputtype = "radians", plotMean = TRUE, plotMoments = 0, col = rgb(0,0,0, alpha), marginSize = 0.1) {
...
ggplotCircular <- function(th, alpha = 0.35, inputtype = "radians",
  ...
  p <- ggplot(aes(x = xsq, y = ysq), data = dsq) +
    geom_path(col = "gray") + coord_fixed() +
